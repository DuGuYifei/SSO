...
public class TCPServerFunctions {
     ...
     */
    public static void handleClient(Socket clientSocket) throws IOException, InterruptedException {
        UUID userId = null;
        ...
        while ((bytesRead = inputStream.read(buffer)) != -1) {
            ...
            if (clientMessage.equals("get-log")) {
                ...
                List<Log> logs;
                logs = logService.findUserLogs(userId, offset, limit);
            ...
            if (clientMessage.equals("live-get-log")) {
                ...
                while (liveCapture) {
                    List<Log> logs;
                    logs = logService.findUserLiveLogs(userId, instant);
                    System.out.println(logs.size());
                    outputStream.writeInt(logs.size());
